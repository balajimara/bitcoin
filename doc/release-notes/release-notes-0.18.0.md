...
`contrib/init/coordinated.service`, it has been changed to use
`/var/lib/coordinated` as the data directory instead of
`~bitcoin/.bitcoin`. When switching over to the new configuration file,
please make sure that the filesystem on which `/var/lib/coordinated` will
exist has enough space (check using `df -h /var/lib/coordinated`), and
...
The systemd init file (`contrib/init/coordinated.service`) has been changed
to use `/var/lib/coordinated` as the data directory instead of
...
- `generate` is deprecated and will be fully removed in a subsequent
  ...
  in this version, restart coordinated with the `-deprecatedrpc=generate`
...
- #14162 Also log and print messages or questions like coordinated (MarcoFalke)
...
- #14521 qt, docs: Fix `coordinate-qt -version` output formatting (hebasto)
