...
be used in V0.17 by starting coordinated with the '-deprecatedrpc=accounts'
...
- `signrawtransaction` is deprecated and will be fully removed in v0.18. To use
  `signrawtransaction` in v0.17, restart coordinated with
...
- When running coordinated with `-debug` but without `-daemon`, logging to stdout
...
meaning the `-txindex` flag can be toggled without a full reindex. If coordinated
...
- #13604 `75bea05` Add depends 32-bit arm support for coordinate-qt (TheCharlatan)
...
- #13696 `2ab7208` Add aarch64 qt depends support for cross compiling coordinate-qt (TheCharlatan)
...
- #12102 `7fb8fb4` Apply hardening measures in coordinated systemd service file (Flowdalic)
- #12652 `55f490a` bitcoin-cli: Provide a better error message when coordinated is not running (practicalswift)
...
- #12769 `35eb9d6` Add systemd service to coordinated in debian package (ghost)
...
- #12877 `2b54155` Use coordinated in Tor documentation (knoxcard)
